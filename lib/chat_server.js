 ...
 */
var socketio=require("socket.io");     //这样就可以使用Socket.IO了，并且在底下初始化了一些定义聊天状态的变量
var io;
var guestNumber=1;
var nickNames={};
var nameUsed=[];
...
exports.listen=function(server){
    io=socketio.listen(server);  //启动Socket IO服务器，允许它搭救在已有的HTTP服务器上
    io.set['log level',1];
    io.sockets.on('connection',function(socket){  //定义每个用户连接的处理逻辑
        ...
        joinRoom(socket,'Lobby');   //用户连接上来时把他放入聊天室Lobby里
...
function joinRoom(socket,room){
    socket.join(room);    //让客户进入房间
    currentRoom[socket.id]=room;    //记录用户的当前房间
    socket.emit('joinResult',{rom:room});   //让用户知道他们进入了新的房间
    ...
    if(usersInRoom.length>1){
        var usersInRoomSummary='Users currentiy in'+room+':';
        for(var index in usersInRoom){    //如果不止一个用户在这个房间里，汇总下都是谁
